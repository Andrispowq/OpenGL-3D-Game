...
int main()
{
    ...
    if (!openALDevice)
    {
        PR_LOG_ERROR("Cannot instantiate OpenAL!\n");
    }

    ALCcontext* openALContext = alcCreateContext(openALDevice, nullptr);
    if (!openALContext)
    {
        PR_LOG_ERROR("Cannot instantiate OpenAL context!\n");
    }

    if (!alcMakeContextCurrent(openALContext))
    {
        PR_LOG_ERROR("Cannot make OpenAL context current!\n");
    }
    ...
    {
        PR_LOG_ERROR("Cannot destroy OpenAL!\n");
    }
