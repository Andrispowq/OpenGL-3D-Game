...
void main()
{
	vec3 albedoColour = pow(getColor(albedoMap, material.colour, texture_FS), vec3(gamma));
	
	float metallic = getColor(metallicMap, material.metallic, texture_FS).r;
	float roughness = getColor(roughnessMap, material.roughness, texture_FS).r;
	float occlusion = getColor(occlusionMap, material.occlusion, texture_FS).r;
	
	vec3 emission = getColor(emissionMap, material.emission, texture_FS);
